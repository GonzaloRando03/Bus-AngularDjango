...
@Component({
  selector: 'app-linea3',
  templateUrl: './linea3.component.html',
  styleUrls: ['./linea3.component.css']
})
export class Linea3Component implements OnInit {

  constructor(private _paradaService: ParadaService, private _viajeService: ViajeService ,private router: Router,) { }

  ngOnInit(): void {
    this.getData('Cº Picasso')
  }
...
/* funcion para reservar un viaje donde montamos un mensaje que llega al backend */
  reservar(index:number, especial:boolean, nDia:number){
    ...
    msg1 = `Billete para el ${dia} la línea L-3 en la parada ${calle} a las ${hora} reservado con éxito`
    msg2 = `Billete para el ${dia} la línea L-3 en la parada ${calle} a las ${hora} reservado con éxito`
...
/* funcion para conseguir la hora del viaje */
  ...
  getData(calle:string){
    this._paradaService.getParadas().subscribe(data => {
      ...
      let l3 = this.getL3(paradas)
      console.log(l3)
      let hour = this.getHour(calle, l3)
      this.data = hour
    }, error => {
      console.log(error)
    })
