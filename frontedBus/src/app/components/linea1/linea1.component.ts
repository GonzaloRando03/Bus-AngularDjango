...
@Component({
  selector: 'app-linea1',
  templateUrl: './linea1.component.html',
  styleUrls: ['./linea1.component.css']
})
export class Linea1Component implements OnInit {

  constructor(private _paradaService: ParadaService, private _viajeService: ViajeService ,private router: Router,) { }

  ngOnInit(): void {
    this.getData('Cº Atlántida')
  }
...
/* funcion para reservar un viaje donde montamos un mensaje que llega al backend */
  reservar(index:number, especial:boolean, nDia:number){
    ...
    msg1 = `Billete para el ${dia} la línea L-1 en la parada ${calle} a las ${hora} reservado con éxito`
    msg2 = `Billete para el ${dia} la línea L-1 en la parada ${calle} a las ${hora} reservado con éxito`
...
/* funcion para conseguir la hora del viaje */
  ...
  getData(calle:string){
    this._paradaService.getParadas().subscribe(data => {
      ...
      let l1 = this.getL1(paradas)
      console.log(l1)
      let hour = this.getHour(calle, l1)
      this.data = hour
    }, error => {
      console.log(error)
    })
